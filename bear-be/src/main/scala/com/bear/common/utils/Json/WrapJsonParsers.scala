...
object WrapJsonParsers {
  ...
  private[utils] def quote(s: String, writer: java.io.Writer)(implicit formats: Formats): java.io.Writer = quote(s, new StringWriterAppender(writer))
  ...
  private[utils] def unquote(buf: Buffer): String = {
   ...
   */
  private[utils] class Buffer(in: java.io.Reader, closeAutomatically: Boolean) {
    var offset = 0
    var curMark = -1
    ...
    private[utils] def automaticClose() = if (closeAutomatically) in.close
   ...
   */
  private[utils] object Segments {
    import java.util.concurrent.ArrayBlockingQueue
    import java.util.concurrent.atomic.AtomicInteger

    private[utils] var segmentSize = 1000
    ...
    private[utils] def clear() = segments.clear
  ...
  private[utils] def parseDouble(s: String) = {
    val d = BigDecimal(s)
